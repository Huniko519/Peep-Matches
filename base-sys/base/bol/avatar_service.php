...
final class BOL_AvatarService
{
    ...
    public function deleteUserAvatar( $userId )
    {
        ...
        $event = new PEEP_Event('base.before_user_avatar_delete', array('avatarId' => $avatar->id ));
    ...
    {
        $path = PEEP::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . 'tmp' . DS;
    ...
    public function renameAvatarOriginal( $userId, $oldHash, $newHash )
    {
        ...
        PEEP::getStorage()->renameFile($originalPath, $newPath);
    }
    ...
    public function getDefaultAvatarUrl( $size = 1 )
    {
        ...
        switch ( $size )
        {
            ...
            case 2:
                return PEEP::getThemeManager()->getSelectedTheme()->getStaticImagesUrl(PEEP::getApplication()->isMobile()) . 'no-avatar-big.png';
        }
    ...
    private function getCustomDefaultAvatarUrl( $size = 1 )
    {
        ...
        $conf = json_decode(PEEP::getConfig()->getValue('base', 'default_avatar'), true);

        if ( isset($conf[$size]) )
        {
            $path = PEEP::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . $conf[$size];

            return PEEP::getStorage()->getFileUrl($path);
        }
    ...
    public function setCustomDefaultAvatar( $size, $file )
    {
        ...
        if ( is_uploaded_file($file['tmp_name']) )
        {
            ...
            if ( $storage->copyFile($file['tmp_name'], $dir . $fileName) )
            {
                ...
                $conf[$size] = $fileName;
                PEEP::getConfig()->saveConfig('base', 'default_avatar', json_encode($conf));
    ...
    {
        $url = PEEP::getPluginManager()->getPlugin('base')->getUserFilesUrl() . 'avatars/tmp/';
    ...
    {
        return PEEP::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS;
    }

    public function getAvatarsPluginFilesDir()
    {
        return PEEP::getPluginManager()->getPlugin('base')->getPluginFilesDir() . 'avatars' . DS;
    }
    ...
    public function trackAvatarChangeActivity( $userId, $avatarId )
    {
        ...
        $event = new PEEP_Event('feed.action', array(
                ...
                'string' => PEEP::getLanguage()->text('base', 'avatar_feed_string'),
    ...
    public function getAvatarChangeGalleryItem( $entityType, $entityId, $itemId )
    {
        ...
        $event = new PEEP_Event('base.avatar_change_get_item', $params);
    ...
    {
        $key = PEEP::getSession()->get(self::AVATAR_CHANGE_SESSION_KEY);
    ...
    public function setAvatarChangeSessionKey()
    {
        ...
        {
            $key = uniqid();
            PEEP::getSession()->set(self::AVATAR_CHANGE_SESSION_KEY, $key);
        }
    }
