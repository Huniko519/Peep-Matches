...
final class BOL_UserService
{
    ...
    public function getDisplayName( $userId )
    {
        ...
        $displayName = isset($questionValue[$userId]) ? ( isset($questionValue[$userId][$questionName]) ? $questionValue[$userId][$questionName] : '' ) : PEEP::getLanguage()->text('base', 'deleted_user');
    ...
    public function getDisplayNamesForList( array $userIdList )
    {
        ...
        {
            $resultArray[$value] = PEEP::getLanguage()->text('base', 'deleted_user');
    ...
    public function getUserUrlForUsername( $username )
    {
        ...
        return PEEP::getRouter()->urlForRoute('base_user_profile', array('username' => $username));
    }
    ...
    public function findOnlineList( $first, $count )
    {
        ...
        $permission = PEEP::getEventManager()->getInstance()->call('privacy_check_permission_for_user_list', $eventParams);
    ...
    {
        $event = new PEEP_Event('base.before_save_user', array('dto' => $user));
    ...
    public function markAsFeatured( $userId )
    {
        ...
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_MARK_FEATURED, array('userId' => $userId));
        PEEP::getEventManager()->trigger($event);
    ...
    public function cancelFeatured( $userId )
    {
        ...
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_UNMARK_FEATURED, array('userId' => $userId));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    {
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_UNREGISTER, array('userId' => $userId, 'deleteContent' => $deleteContent));
        PEEP::getEventManager()->trigger($event);
    ...
    public function suspend( $userId, $message )
    {
        ...
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_SUSPEND, array('userId' => $userId, 'message' => $message));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    public function unsuspend( $userId )
    {
        ...
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_UNSUSPEND, array('userId' => $userId));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    public function block( $userId )
    {
        ...
        $dto->setUserId(PEEP::getUser()->getId());
        $dto->setBlockedUserId($userId);

        $this->userBlockDao->save($dto);

        $event = new PEEP_Event(PEEP_EventManager::ON_USER_BLOCK, array('userId' => PEEP::getUser()->getId(), 'blockedUserId' => $userId));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    public function unblock( $userId )
    {
        ...
        $dto = $this->userBlockDao->findBlockedUser(PEEP::getUser()->getId(), $userId);

        $this->userBlockDao->delete($dto);

        $event = new PEEP_Event(PEEP_EventManager::ON_USER_UNBLOCK, array('userId' => PEEP::getUser()->getId(), 'blockedUserId' => $userId));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    {
        return hash('sha256', PEEP_PASSWORD_SALT . $password);
    }
    ...
    {
        $timestamp = PEEP::getConfig()->configExists('base', 'user_expired_time') ? time() - PEEP::getConfig()->getValue('base', 'user_expired_time') * 60 : time() - 30 * 60;
    ...
    public function createUser( $username, $password, $email, $accountType = null, $emailVerify = false )
    {
        ...
        $user->joinIp = ip2long(PEEP::getRequest()->getRemoteAddress());
    ...
    public function sendAdminInvitation( $email )
    {
        ...
        $inviteUrl = PEEP::getRequest()->buildUrlQueryString(PEEP::getRouter()->urlForRoute('base_join'), array('code' => $inviteCodeDto->getCode()));

        $mail = PEEP::getMailer()->createMail();
        $mail->addRecipientEmail($email);
        $mail->setSubject(PEEP::getLanguage()->text('base', 'mail_template_admin_invite_user_subject'));
        $mail->setHtmlContent(PEEP::getLanguage()->text('base', 'mail_template_admin_invite_user_content_html', array('url' => $inviteUrl)));
        $mail->setTextContent(PEEP::getLanguage()->text('base', 'mail_template_admin_invite_user_content_text', array('url' => $inviteUrl)));
    ...
    public function disapprove( $userId )
    {
        ...
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_DISAPPROVE, array('userId' => $userId));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    public function approve( $userId )
    {
        ...
        $event = new PEEP_Event(PEEP_EventManager::ON_USER_APPROVE, array('userId' => $userId));
        PEEP::getEventManager()->trigger($event);
    }
    ...
    public function findUserIdListByQuestionValues( $questionValues, $first, $count, $isAdmin = false, $aditionalParams = array() )
    {
        ...
        $event = new PEEP_Event("base.question.before_user_search", $data, $data);
    ...
    public function sendWellcomeLetter( BOL_User $user )
    {
        ...
        if ( PEEP::getConfig()->getValue('base', 'confirm_email') && $user->emailVerify != true )
        {
            return;
        }
    ...
    public function processSignIn( $identity, $password, $rememberMe = false )
    {
        ...
        $result = PEEP::getUser()->authenticate(new BASE_CLASS_StandardAuth($identity, $password));

        if ( $result->isValid() )
        {
            if ( $rememberMe )
            {
                $loginCookie = $this->saveLoginCookie(PEEP::getUser()->getId());

                setcookie('peep_login', $loginCookie->getCookie(), (time() + 86400 * 7), '/', null, null, true);
            }
        }
    ...
    public function processResetForm( $data )
    {
        ...
        $vars = array('code' => $resetPassword->getCode(), 'username' => $user->getUsername(), 'requestUrl' => PEEP::getRouter()->urlForRoute('base.reset_user_password_request'),
            'resetUrl' => PEEP::getRouter()->urlForRoute('base.reset_user_password', array('code' => $resetPassword->getCode())));
    ...
    public function processResetPasswordForm( $data, BOL_User $user, $resetCode )
    {
        ...
        {
            throw new LogicException(PEEP::getLanguage()->text('base', 'reset_password_length_error_message', array('min' => UTIL_Validator::PASSWORD_MIN_LENGTH, 'max' => UTIL_Validator::PASSWORD_MAX_LENGTH)));
        }
    ...
    public function getUserViewQuestions( $userId, $adminMode = false, $questionNames = array(), $sectionNames = null )
    {
        ...
        foreach ( $questionArray as $sectionKey => $section )
        {
            ...
            {
                $event = new PEEP_Event('base.questions_field_get_label', array(
                ...
                $event = new PEEP_Event('base.questions_field_get_value', array(
                    'presentation' => $question['presentation'],
                    'fieldName' => $question['name'],
                ...
                if ( !empty($questionData[$userId][$question['name']]) )
                {
                    switch ( $question['presentation'] )
                    {
                        case BOL_QuestionService::QUESTION_PRESENTATION_CHECKBOX:
                            ...
                            {
                                $questionData[$userId][$question['name']] = PEEP::getLanguage()->text('base', 'yes');
                            }
                            else
                            {
