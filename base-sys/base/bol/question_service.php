...
class BOL_QuestionService
{
    ...
    public function getPresentationClass( $presentation, $fieldName, $configs = null )
    {
        ...
        $event = new PEEP_Event('base.questions_field_init', array(
            'type' => 'main',
            'presentation' => $presentation,
            'fieldName' => $fieldName,
            'configs' => $configs
    ...
    public function getFormElementByQuestionDto( BOL_Question $question, array $values, $type = 'join' )
    {
        ...
        if ( !empty($class) )
        {
            $class->setLabel(PEEP::getLanguage()->text('base', 'questions_question_' . $question->name . '_label'));
            ...
            if ( in_array($question->type, array(BOL_QuestionService::QUESTION_VALUE_TYPE_SELECT, BOL_QuestionService::QUESTION_VALUE_TYPE_MULTISELECT) ) )
            {
                ...
                if ( method_exists($class, 'setOptions') )
                {
                    if ( !empty($values[$question->name]['values']) && is_array($values[$question->name]['values']) )
                    {
                        ...
                        {
                            $valuesArray[$value->value] = PEEP::getLanguage()->text( 'base', 'questions_question_' . $value->questionName . '_value_' . ($value->value) );
                        }
    ...
    public function deleteQuestionValue( $questionName, $value )
    {
        ...
        if ( $isDelete )
        {
            ...
            $event = new PEEP_Event(self::EVENT_AFTER_DELETE_QUESTION_VALUE, array('questionName' => $questionName, 'value' => $value));
            PEEP::getEventManager()->trigger($event);
        }
    ...
    public function deleteAccountType( $accountType )
    {
        ...
        $deleted = (boolean) PEEP::getDbo()->getAffectedRows();

        if ( $deleted )
        {
            $event = new PEEP_Event(self::EVENT_ON_ACCOUNT_TYPE_DELETE, array('id' => $account->id, 'name' => $account->name));
            PEEP::getEventManager()->trigger($event);
        }
    ...
    public function deleteQuestion( array $questionIdList )
    {
        ...
        foreach ( $questionArray as $question )
        {
            ...
            $event = new PEEP_Event( self::EVENT_ON_QUESTION_DELETE, array( 'questionName' => $question->name, 'dto' => $question ) );
        ...
        return (boolean) PEEP::getDbo()->getAffectedRows();
    }
    ...
    public function saveQuestionsData( array $data, $userId )
    {
        ...
        $event = new PEEP_Event('base.questions_save_data', array('userId' => $userId), $data);
        ...
        if ( $sendVerifyMail && PEEP::getConfig()->getValue('base', 'confirm_email') )
        {
    ...
    public function getQuestionData( array $userIdList, array $fieldsList )
    {
        ...
        $event = new PEEP_Event('base.questions_get_data', array('userIdList' => $userIdList, 'fieldsList' => $fieldsList), $result);
    ...
    public function getQuestionLangKeyName( $type, $name, $value = null )
    {
        ...
        $event = new PEEP_Event( self::EVENT_ON_GET_QUESTION_LANG, array('type' => $type, 'name' => $name, 'value' => $value));
        PEEP::getEventManager()->trigger($event);
    ...
    public function getQuestionDescriptionLang( $questionName )
    {
        ...
        $text = PEEP::getLanguage()->text(self::QUESTION_LANG_PREFIX,$key);
    ...
    {
        return PEEP::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_QUESTION_LABEL, $questionName));
    }

    public function getQuestionValueLang( $questionName, $value )
    {
        return PEEP::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_QUESTION_VALUE, $questionName, $value));
    }

    public function getSectionLang( $sectionName )
    {
        return PEEP::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_QUESTION_SECTION, $sectionName));
    }

    public function getAccountTypeLang( $accountType )
    {
        return PEEP::getLanguage()->text(self::QUESTION_LANG_PREFIX, $this->getQuestionLangKeyName(self::LANG_KEY_TYPE_ACCOUNT_TYPE, $accountType));
    }
    ...
    public function createQuestion( BOL_Question $question, $label, $description = '', $values = array(), $saveValuesKeys = false, $generateCache = true )
    {
        ...
        if ( $generateCache )
        {
            BOL_LanguageService::getInstance()->generateCache(PEEP::getLanguage()->getCurrentId());
        }
...
//                    BOL_LanguageService::getInstance()->addOrUpdateValue(PEEP::getLanguage()->getCurrentId(), 'base', 'questions_question_' . ($questionName) . '_value_' . $value, $label, $generateCache);
                    $event = new PEEP_Event(self::EVENT_AFTER_UPDATE_QUESTION_VALUE, array('dto' => $existingValuesList[$key]));
                    PEEP::getEventManager()->trigger($event);
                }
                else
                {
    ...
    public function addQuestionValue( $questionName, $value, $label, $sortOrder, $generateCache = true )
    {
        ...
        $event = new PEEP_Event(self::EVENT_AFTER_ADD_QUESTION_VALUE, array('dto' => $questionValue));
        PEEP::getEventManager()->trigger($event);
    ...
    public function updateQuestionsEditStamp()
    {
        ...
        {
            PEEP::getConfig()->saveConfig( 'base', 'profile_question_edit_stamp', time() );
    ...
    {
        return PEEP::getConfig()->getValue( 'base', 'profile_question_edit_stamp' );
    }
    ...
    public function getQuestionDisableActionList( BOL_question $question )
    {
        ...
        $event = new PEEP_Event( 'admin.disable_fields_on_edit_profile_question', array( 'questionDto' => $question ), $disableActionList );
        PEEP::getEventManager()->trigger($event);
    ...
    public function createAccountType( $accountTypeName, $label = '', $roleId = 0 )
    {
        ...
        $event = new PEEP_Event(self::EVENT_ON_ACCOUNT_TYPE_ADD, array('dto' => $accountType, 'id' => $accountType->id));
        PEEP::getEventManager()->trigger($event);

        $questionNameList = $this->getRequiredQuestionsForNewAccountType();
        
        $event = new PEEP_Event(self::EVENT_BEFORE_ADD_QUESTIONS_TO_NEW_ACCOUNT_TYPE, array('dto' => $accountType), $questionNameList);
        PEEP::getEventManager()->trigger($event);
    ...
    public function getQuestionValueForUserList( BOL_Question $question, $value )
    {
        ...
        switch ( $question->presentation )
        {
            case BOL_QuestionService::QUESTION_PRESENTATION_CHECKBOX:
                ...
                if ( (int) $value === 1 )
                {
                    $stringValue = PEEP::getLanguage()->text('base', 'yes');
                }
