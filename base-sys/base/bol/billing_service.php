...
final class BOL_BillingService
{
    ...
    public function initSale( BOL_BillingSale $sale, $gatewayKey )
    {
        ...
        $event = new PEEP_Event(self::EVENT_ON_AFTER_INIT_SALE, array("saleDbo" => $sale, "gateway" => $gateway));
     ...
     * @return bool
     */
    public function prepareSale( PEEP_BillingAdapter $adapter, BOL_BillingSale $sale )
    {
     ...
     * 
     * @param PEEP_BillingAdapter $adapter
     * @param BOL_BillingSale $sale
     * @return boolean
     */
    public function verifySale( PEEP_BillingAdapter $adapter, BOL_BillingSale $sale )
    {
        if ( !$this->getSaleById($sale->id) )
        {
            return false;
     ...
     * 
     * @param PEEP_BillingProductAdapter $adapter
     * @param BOL_BillingSale $sale
     * @return int
     */
    public function deliverSale( PEEP_BillingProductAdapter $adapter, BOL_BillingSale $sale )
    {
        if ( !$this->getSaleById($sale->id) )
        {
            return false;
        ...
        if ( $adapter->deliverSale($sale) )
        {
            ...
            $this->saveSale($sale);
            $event = new PEEP_Event(self::EVENT_ON_AFTER_DELIVER_SALE, array("saleDbo" => $sale, "adapter" => $adapter));
            PEEP::getEventManager()->trigger($event);
            return true;
        }
    ...
    public function registerRebillSale( PEEP_BillingAdapter $adapter, BOL_BillingSale $parentSale, $rebillTransId )
    {
        $parentSale->id = null;
     ...
     * 
     * @param string $productKey
     * @return PEEP_BillingProductAdapter
     */
    ...
    {
        return PEEP::getConfig()->getValue('base', 'billing_currency');
    }
    ...
    public function getOrderCompletedPageUrl( $hash = null )
    {
        ...
        {
            return PEEP::getRouter()->urlForRoute('base_billing_completed', array('hash' => $hash));
        }
        else
        {
            return PEEP::getRouter()->urlForRoute('base_billing_completed_st');
        }
    }
    ...
    public function getOrderCancelledPageUrl( $hash = null )
    {
        if ( isset($hash) && $sale = $this->getSaleByHash($hash) )
        {
            return PEEP::getRouter()->urlForRoute('base_billing_canceled', array('hash' => $hash));
        }
        else
        {
            return PEEP::getRouter()->urlForRoute('base_billing_canceled_st');
        }
    }
    ...
    {
        return PEEP::getRouter()->urlForRoute('base_billing_error');
    }
