...
class ADMIN_CLASS_AddAccountTypeForm extends Form {
    ...
    public function __construct(BOL_QuestionAccountType $accountType, $formName = '') {
        ...
        $this->setAjax(true);
        $this->setAction(PEEP::getRouter()->urlFor("ADMIN_CTRL_Questions", "ajaxResponder"));
        ...
        $submit->setValue(PEEP::getLanguage()->text('admin', 'questions_add_new_account_type'));

        $jsString = ' peepForms[{$formName}].bind("success", function(json){
            if ( json.result.add == true) {
                PEEP.registerLanguageKey("base", ' . json_encode($key) . ', json.accountTypeName);
                PEEP.trigger("admin.add_account_type", [json], this);
            }
        }); ';

        PEEP::getLanguage()->addKeyForJs($prefix, $key);
        if (!empty($accountType->id)) {
            $jsString = ' peepForms[{$formName}].bind("success", function(json){
                if ( json.result.update == true) {
                    PEEP.registerLanguageKey("base", ' . json_encode($key) . ', json.accountTypeName);
                    PEEP.trigger("admin.update_account_type", [json], this);
                }
            }); ';
        }
        ...
        if ($accountType->id) {
            $submit->setValue(PEEP::getLanguage()->text('admin', 'questions_save_account_type'));
        }
    ...
    protected function saveOrUpdateAccountType($accountTypeName, $roleId, $order) 
    {
        ...
        if (isset($order)) {
            ...
            if ( !empty($direction) )
            {
                ...
                $event = new PEEP_Event(BOL_QuestionService::EVENT_ON_ACCOUNT_TYPE_REORDER, array('dto' => $accountType, 'id' => $accountType->id, 'orderList' => $list));
    ...
    protected function saveLangValues($prefix, $key) {
        ...
        $languageService->generateCache(PEEP::getLanguage()->getCurrentId());
    }
