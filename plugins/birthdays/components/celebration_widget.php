...
class BIRTHDAYS_CMP_CelebrationWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $params )
    {
        ...
        $this->assign('celebrationSrc', PEEP::getPluginManager()->getPlugin('birthdays')->getStaticUrl().'img/' . 'celebration.png');
        ...
        {
            $label = '<span class="peep_green" style="font-weight: bold; font-size:15px; text-align:center; position:relative; bottom:-5px;">' . PEEP::getLanguage()->text('base', 'date_time_today') . '</span> ';
 
        }
        else if ( $dateInfo['day'] == date('d') + 1 )
        {
            $label = '<span class="peep_green" style="font-weight: bold; font-size:15px; text-align:center; position:relative; bottom:-5px;">' . PEEP::getLanguage()->text('base', 'date_time_tomorrow') . '</span> ';

        }
        else
        {
            $label = '<span class="peep_green" style="font-weight: bold; font-size:15px; text-align:center; position:relative;  bottom:-5px;">' . UTIL_DateTime::formatBirthdate($dateInfo['year'], $dateInfo['month'], $dateInfo['day']) . '</span>';
        }
