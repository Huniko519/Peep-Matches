(function( window, $, params ) {'use strict';
    ...
    _methods = {
        ...
        cachePhotoComponents: function( photos )
        {
            ...
            keys.forEach(function( item )
            {
                if ( !_methods.isCached(item) )
                {
                    ...
                    PEEP.trigger('photo.afterPhotoCached', [cmp.photo.id, cmp], _methods);
                }
            });
        ...
        {
            PEEP.trigger('photo.onBeforeShow', [photoId, listType], _methods);
            
            if ( photoId !== _vars.photoId )
            {
                PEEP.trigger('photo.onBeforePhotoChange', [_vars.photoId, photoId], _methods);
            }
            ...
            if ( data && data.main && data.main.length === 2 && data.mainUrl )
            {
                ...
                PEEP.trigger('photo.setStage', [photo, data, stageCss, imgCss], _elements.content);
        ...
        fetchCmp: function( dataToSend, params )
        {
            _vars.fetchRequest = $.ajax(
            {
                ...
                success : function( responce )
                {
                    ...
                    PEEP.trigger('photo.onRequestComplete', [dataToSend, params, responce], _methods);
        ...
        loadCachedComponent: function( photoId )
        {
            ...
            {
                var contant = $(document.getElementById('peep-photo-view-page'));
            ...
            _methods.fitSize(photoId, function( offset )
            {
                ...
                if ( !_methods.fullscreen.isFullscreen() )
                {
                    if ( _vars.layout !== 'page' )
                    {
                        ...
                        if ( _vars.isClassic )
                        {
                            _elements.content.find('.peep_photoview_stage_wrap').css({float: 'none', height: _vars.minSize.height});
                            options.width = offset.width;
                        }
                        else
                        {
                            _elements.content.find('.peep_photoview_stage_wrap').css({width: _vars.minSize.width, height: _vars.minSize.height});
                ...
                PEEP.trigger('photo.photo_show', {photoId: photoId}, _methods);
        ...
        fitSize: function( photoId, callback, resize )
        {
            ...
            var complete = function()
            {
                ...
                {
                    var img = _elements.content.find('img.peep_photo_view').css({width: 'auto', height: 'auto'})[0];
                ...
                var screnWeight = _vars.layout === 'page' ? $(document.getElementById('peep-photo-view-page')).width() : $(window).width();
        ...
        {
            var screenWidth = _vars.layout === 'page' ? $(document.getElementById('peep-photo-view-page')).width() : $(window).width(),
        ...
        setContent: function( photoId )
        {
            ...
            $('.peep_user_list_picture img', content).attr({alt: cmp.avatar.title, src: cmp.avatar.src}).show();
            ...
            $('.peep_user_list_data .peep_photo_album_url', content).attr('href', cmp.albumUrl)
                .find('.peep_photo_album_name').html(
                    PEEP.getLanguageText('photo', 'album') + ': ' + utils.truncate(cmp.album.name, 60)
            );
            ...
            $('#btn-photo-edit').off().on('click', function()
            {
                ...
                _elements.editFB = PEEP.ajaxFloatBox('PHOTO_CMP_EditPhoto', {photoId: photoId}, {width: 580, iconClass: 'peep_ic_edit', title: PEEP.getLanguageText('photo', 'tb_edit_photo'),
                    onLoad: function()
                    {
                            ...
                            if ( data && data.result )
                            {
                                ...
                                else
                                {
                                    ...
                                    $('.peep_user_list_data .peep_photo_album_url', content).attr('href', data.albumUrl)
                                        .find('.peep_photo_album_name').html(
                                            PEEP.getLanguageText('photo', 'album') + ': ' + utils.truncate(data.albumName, 60)
                                        );
                ...
                $.ajax(
                {
                    ...
                    success : function(data)
                    {
                        if ( data.result === true )
                        {
                            ...
                            var newLabel = status === 'remove_from_featured' ? PEEP.getLanguageText('photo', 'mark_featured') : PEEP.getLanguageText('photo', 'remove_from_featured');
                            $this.html(newLabel);
            ...
            if ( !_methods.fullscreen.enabled() )
            {
                _elements.content.find('.peep_photoview_fullscreen').attr({target: '_blank', href: cmp.photo.url});
            }
            ...
            var rateItems = $('.peep_rates_wrap', content).show().find('.rate_item').off();
            
            if ( +userScore > 0 )
            {
                content.find('.rate_title').html(
                    PEEP.getLanguageText('photo', 'rating_your', {count: rateInfo.rates_count, score: userScore})
                );
            }
            else
            {
                content.find('.rate_title').html(
                    PEEP.getLanguageText('photo', 'rating_total', {count: rateInfo.rates_count})
                );
            }
            ...
            PEEP.trigger('photo.canRate', eventRate, this.node);
                            ...
                            if ( result )
                            {
                                switch ( result.result )
                                {
                                    case true:
                                        ...
                                        content.find('.rate_title').html(
                                            PEEP.getLanguageText('photo', 'rating_your', {count: result.rateInfo.rates_count, score: rate})
                                        );
        ...
        updateComment: function( data, eventName )
        {
            ...
            {
                PEEP.addScroll(scrollCont, {autoReinitialise: true}).scrollToBottom();
        ...
        loadPhoto: function( direction, listType )
        {
            ...
            PEEP.trigger('photo.onPhotoSwitch', [photoId, _vars.direction], _methods);
        ...
        fullscreen: {
            ...
            onchange: function( event )
            {
                ...
                if ( _methods.fullscreen.isFullscreen() )
                {
                    ...
                    content.find('.peep_photoview_info_wrap').css({height: screen.height});
                    ...
                    if ( _vars.layout === 'page' || _vars.isClassic )
                    {
                        ...
                        content.find('.peep_photoview_stage_wrap').css({float: 'left', width: screen.width, height: screen.height});
                    ...
                    else
                    {
                        ...
                        content.find('.peep_photoview_stage_wrap').css({width: screen.width, height: screen.height});
                    }
                ...
                PEEP.trigger('photo.onSizeChange', [_vars.photoId, _methods.fullscreen.isFullscreen()], _methods);
            }
        },
        utils:
        {
            showPreloader: function()
            {
                ...
                {
                    if ( !_elements.photoFB.$canvas.hasClass('peep_floatbox_loading') )
        ...
        arrayUtils:
        {
            unionToList: function()
            {
                ...
                var event = {listType: []};
                PEEP.trigger('photo.collectListType', event, _methods);
        ...
        createFloatboxContent: function()
        {
            ...
            _elements.content.find('.peep_photoview_slide_time').slider({min: 3, max:10, range: 'min',
                slide: function( event, ui )
                {
                    var tip = $(this).data('peepTip');
            ...
            _vars.timerId = setTimeout(function()
            {
                _methods.fitSize(_vars.photoId, function( offset )
                {
                    ...
                    if ( _vars.layout !== 'page' )
                    {
                        ...
                        if ( _vars.isClassic )
                        {
                            _elements.content.find('.peep_photoview_stage_wrap').css({float: 'none', height: _vars.minSize.height, width: ''});
                            options.width = offset.width;
                        }
                        else
                        {
                    ...
                    {
                        _elements.content.find('.peep_photoview_stage_wrap').css({height: offset.height, width: offset.width});
                    }
        ...
        runSlideshow: function()
        {
            ...
            _vars.slideshowTimeId = setInterval(function()
            {
                ...
                else
                {
                    ...
                    var img = _elements.content.find('img.peep_photo_view');
        ...
        init: function()
        {
            ...
            var content = $(document.getElementById('peep-photo-view'));
            ...
            if ( _methods.fullscreen.enabled() )
            {
                ...
                $('.peep_photoview_info_btn', content).on('click', function( event )
                {
                    ...
                    if ( $(this).hasClass('open') )
                    {
                        ...
                        var wrap = _elements.content.find('.peep_photoview_stage_wrap');
                ...
                $('.peep_photoview_slide_settings', content).on({
                    mouseenter: function()
                    {
                        ...
                        $('.peep_photoview_slide_settings_controls', _elements.content).stop(true).css({display: 'inline-block', opacity: 1});
                    }
                });
