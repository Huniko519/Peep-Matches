...
questionValuesField.prototype = {
    ...
    construct : function ( params )
    {
        ...
        self.valuesArea.on( 'click', '.remove', function() {
            if( confirm(PEEP.getLanguageText('admin', 'questions_edit_delete_value_confirm_message')) )
            {
    ...
    setValue : function ( values )
    {
        ...
        if ( list )
        {
            ...
            self.renderValues();
            PEEP.trigger('question.value.add', {values:addValues, node:self.tr});
    ...
    deleteValue : function ( element )
    {
        ...
        div.remove();
        PEEP.trigger('question.value.delete', {value:item, node:self.tr});
        self.updateDataField();
    },
...
questionPagesModel.prototype = {
    ...
    {
        var self = this;
        self.setRequired( self.required_node.hasClass('peep_checkbox_cell_marked') );
        self.setJoin( self.join_node.hasClass('peep_checkbox_cell_marked') );
        self.setEdit( self.edit_node.hasClass('peep_checkbox_cell_marked') );
        self.setView( self.view_node.hasClass('peep_checkbox_cell_marked') );
        self.setSearch( self.search_node.hasClass('peep_checkbox_cell_marked') );
    },
    ...
    change : function( element )
    {
        ...
        switch( true )
        {
            case  ( element.hasClass('required') ) :
                self.setRequired( element.hasClass('peep_checkbox_cell_marked') );
                changes = 'required';
            break;

            case ( element.hasClass('on_join') ) :
                self.setJoin( element.hasClass('peep_checkbox_cell_marked') );
                changes = 'onJoin';
            break;

            case ( element.hasClass('on_edit') ) :
                self.setEdit( element.hasClass('peep_checkbox_cell_marked') );
                changes = 'onEdit';
            break;

            case ( element.hasClass('on_search') ) :
                self.setSearch( element.hasClass('peep_checkbox_cell_marked') );
                changes = 'onSearch';
            break;

            case ( element.hasClass('on_view') ) :
                self.setView( element.hasClass('peep_checkbox_cell_marked') );
                changes = 'onView';
            break;
        }
...
var indexQuestions = function( $params )
{
    ...
    var $questionDiv = $('.peep_admin_profile_questions_list_div');
    var $questionTable = $('.peep_admin_profile_questions_list');
    ...
    $('a.question_edit_button').click( function()  {
        ...
        if ( questionId )
        {
            PEEP.ajaxFloatBox('ADMIN_CMP_EditQuestion', [questionId], {
                width: '700px',
                title: PEEP.getLanguageText('admin', 'questions_edit_profile_question_title')
            } );
        }
    } );
    ...
    $('a.add_account_type').click( function()  {
        ...
        PEEP.ajaxFloatBox('ADMIN_CMP_AddAccountType', [], {
            width: '500px',
            title: PEEP.getLanguageText('admin', 'questions_add_account_type_title')
        } );
    } );
    ...
    $('a.question_edit_account_type_button').click( function()  {
        ...
        if ( input.length > 0 && input.val() )
        {
            PEEP.ajaxFloatBox('ADMIN_CMP_EditAccountType', [input.val()], {
                width: '500px',
                title: PEEP.getLanguageText('admin', 'questions_edit_account_type_title')
            } );
        }
    } );
   ...
   $('a.question_delete_account_type_button').click(
        function(event) {
            ...
            if( confirm(PEEP.getLanguageText('admin', 'questions_delete_account_type_confirmation')) )
            {
                self.sendRequest = true;
                $.ajax( {
                    url: self.responderUrl,
    ...
    $('input.add_new_question_button').click( function()  {
        ...
        PEEP.ajaxFloatBox('ADMIN_CMP_AddQuestion', [], {
            width: '700px',
            title: PEEP.getLanguageText('admin', 'questions_add_profile_question_title')
        } );
    } );
    ...
    $('input.add_new_section_button').on( 'click', function()  {
        PEEP.ajaxFloatBox('ADMIN_CMP_AddQuestionSection', [], {
            width: '600px',
            title: PEEP.getLanguageText('admin', 'questions_profile_question_sections_title')
        } );
    } );
    ...
    $('.question_delete_button').click(
        function()
        {
            ...
            if( confirm(PEEP.getLanguageText('admin', 'questions_delete_question_confirmation_' + $questionId)) )
            {
                self.sendRequest = true;
                $.ajax( {
                    url: self.responderUrl,
        ...
        function() {
            ...
            .done(
                function ( result ) {
                    ...
                    var message = PEEP.getLanguageText('admin', 'questions_delete_section_confirmation');
    ...
    $(".edit_sectionNameLang").click( function() {
         ...
         var $element = $tr.find(".section_value .peep_section_label");
         var $name = $(this).parents(".peep_admin_profile_questions_list:eq(0)").attr("sectionName");
     ...
     $questionDiv.sortable({
       ...
       update: function(event, ui)
       {
            ...
            $questionDiv.find('.peep_admin_profile_questions_list:not(.no_section)').each(function(ord, o){
        ...
        helper: function(event, ui)
        {
            ...
            return $('<div class="peep_dnd_helper" style="width: 180px;height: 30px; text-align:center; vertical-align:middle;"></div>');
        }
     ...
     $questionTable.not(".about_my_match").sortable(
     {
       ...
       connectWith: '.peep_admin_profile_questions_list:not(.no_section):not(.about_my_match)',
        ...
        update: function(event, ui) {
             ...
             var newSection = ui.item.parents(".peep_admin_profile_questions_list:eq(0)");
