var avatarChange = function( params )
{
    ...
    this.initCrop = function( callback ){
        ...
        image.onload = function(e) {
            ...
            $(".peep_avatar_crop_area").css({ width: cropWidth });

            $(".peep_avatar_crop_result").css({ width: self.params.minCropSize });
    ...
    {
        self.$backButton.closest(".peep_button").hide();

        // join form
        if ( self.params.inputId ) {

            self.$step2.hide();
            PEEP.inProgressNode(self.$preloader);
            self.$preloader.show();
            ...
            if ( file ) {
                ...
                reader.onload = function(e) {
                    ...
                    image.onload = function(e) {
                        ...
                        var height = this.height;
                        if (width < self.params.minCropSize || height < self.params.minCropSize) {
                            PEEP.error(PEEP.getLanguageText('base', 'avatar_image_too_small', { width : self.params.minCropSize, height : self.params.minCropSize }));
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.result) 
                {
                    if ( !self.params.inputId ) 
                    {
                        PEEP.info(PEEP.getLanguageText('base', 'avatar_changed'));
                    }

                    PEEP.trigger('base.avatar_cropped', data); // Listen this event to update interface
                    document.avatarFloatBox.close();
                }
                else 
                {
                    ...
                    {
                        PEEP.error(PEEP.getLanguageText('base', 'crop_avatar_failed'));
                    }
                }
    ...
    this.$backButton.click(function(){
        self.$step2.hide();
        self.$step1.show();
        document.avatarFloatBox.$header.find(".floatbox_title span").html(PEEP.getLanguageText('base', 'avatar_change'));
    });
    ...
    self.$fileInput.change(function(e){
        ...
        if ( !files || !self.checkFileType(files[0].type) ) {
            PEEP.error(PEEP.getLanguageText('base', 'not_valid_image'));
    ...
    this.$dropArea.on('drop', function(e){
        ...
        if (files && files.length > 1) {
            PEEP.error(PEEP.getLanguageText('base', 'avatar_drop_single_image'));
        }
        else {
            if ( !files || !self.checkFileType(files[0].type) ) {
                PEEP.error(PEEP.getLanguageText('base', 'not_valid_image'));
    ...
    $("body").on("click", ".peep_photo_avatar_hover", function(){
        var image = new Image();
        var $btn = $(this).find(".avatar_select");
        var node = $(this).parents(".peep_photo_item:eq(0)");
        
        $btn.hide();
        node.addClass("peep_avatar_preloader");
        var timeout;
        
        function hidePreolader () {
            node.removeClass("peep_avatar_preloader");
            $btn.show();
...
//    });
    ...
    $(".avatar_load_more").click(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.result) {
                    var $item = $parent.closest(".peep_photo_item_wrap");
