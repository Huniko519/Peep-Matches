...
PEEP_Console = function( params, modelsData )
{
    ...
    var command = PEEP.getPing().addCommand('consoleUpdate',
    {
        before: function()
        {
            var c = this;
...
PEEP_ConsoleDropdownHover = function( uniqId, contentUniqId )
{
...
PEEP_ConsoleDropdownClick = function( uniqId, contentUniqId )
{
    this.init( uniqId, contentUniqId );

    this.initDropdown();
}
$.extend(PEEP_ConsoleDropdownClick.prototype, PEEP_ConsoleItem,
{
...
PEEP_ConsoleDropdownList = function( uniqId, contentUniqId )
{
    this.init( uniqId, contentUniqId );
    this.initDropdown();

    var $counter = $('.PEEP_ConsoleItemCounter', this.$node),
    ...
    var numberSetActive = function( active )
    {
        ...
        $place[ active ? 'addClass' : 'removeClass' ]('peep_count_active');
    }
    ...
    var numberShow = function( number, animate )
    {
        ...
        var numberHeight = $number.outerHeight();
        $place.animate({height: numberHeight}, 'fast');

        if ( animate )
        {
            $number.css({top: -placeHeight}).animate({top: 0}, 'fast');
        }
    };
    ...
    var numberHide = function( callBack )
    {
        ...
        $number.animate({top: placeHeight}, 'fast', callBack);
    };
    ...
    var counterShow = function( callBack )
    {
        ...
        $place.css({height: 2}).animate({height: placeHeight}, 'fast', callBack);
 ...
 var counterHide = function( callBack )
    {
        numberHide(function()
        {
            $place.animate({height: 0}, 'fast', function()
            {
                $number.text(0);
...
PEEP_ConsoleList =
{
    construct: function( params )
    {
        ...
        this.$container = function() {
            return $('.PEEP_ConsoleListContainer', self.$contentNode);
        };

        this.$list = function() {
            return $('.PEEP_ConsoleList', self.$container());
        };

        this.$preloader = function() {
            return $('.PEEP_ConsoleListPreloader', self.$container());
        };

        this.$noContent = function() {
            return $('.PEEP_ConsoleListNoContent', self.$container());
        };
    ...
    _loadList: function()
    {
        ...
        var request = JSON.stringify({
            console: PEEP.Console.getData('console').get(),
            data: this.data.get(),
            target: target,
    ...
    updateScroll: function( toTop )
    {
        ...
        if ( this.opened )
        {
            ...
            var jsp = PEEP.addScroll(this.$container().get(0));
...
PEEP_Invitation = function( itemKey, params )
{
