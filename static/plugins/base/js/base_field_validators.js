        ...
        this.validateUsername = function( $element )
        {
            ...
            if( username.val().length > 0 )
            {
                if( self.errors['username']['value'] !==  username.val() )
                {
                    ...
                    var element = peepForms[this.formName].elements[username.attr('name')];
                    ...
                    if( !this.usernamePattern.test( username.val() ) )
                    {
                        ...
                        element.showError( PEEP.getLanguageText('base', 'join_error_username_not_valid') );
                        return false;
                    }
                    else
                    {
                        $.ajax( {
                            ...
                            success: function( data )
                            {
                                if( data.result == false )
                                {
                                     ...
                                     element.showError( PEEP.getLanguageText('base', 'join_error_username_already_exist') );
                                }
                            }
                        } );
                    }
        ...
        this.validateEmail = function( $element )
        {
            ...
            if( email.val().length > 0 )
            {
                if( self.errors['email']['value'] !== email.val() )
                {
                    ...
                    var element = peepForms[this.formName].elements[email.attr('name')];
                    element.removeErrors();

                    if( !this.emailPattern.test( email.val() ) )
                    {
                        self.errors['email']['error'] = PEEP.getLanguageText('base', 'join_error_email_not_valid');
                        element.showError( PEEP.getLanguageText('base', 'join_error_email_not_valid') );
                        return false;
                    }
                    else
                    {
                                    ...
                                    $.ajax( {
                                            ...
                                            success: function( data )
                                            {
                                                if( data.result == false )
                                                {
                                                     ...
                                                     element.showError( PEEP.getLanguageText('base', 'join_error_email_already_exist') );
                                                }
                                            }
                                    } );
//
...
//                            }
        ...
        {
            var element = peepForms[this.formName].elements['password'];
            element.removeErrors();
