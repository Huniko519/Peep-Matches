var PeepUtils = function(){
    var langs = {};
    var messageTime = 3000;
    var $messageCont = $('<div class="peep_message_cont"></div>');
    var events = {};
    ...
    this.message = function( message, type, paramTime ){
        var $messageNode = $('<div class="peep_message_node '+type+'" style="display:none;"><div><div><a class="close_button" href="javascript://" onclick="$(this).closest(\'.peep_message_node\').slideUp(200, function(){$(this).remove();})"></a>'+message+'</div></div></div>').appendTo($messageCont);
        if( paramTime == undefined ){
            paramTime = messageTime;
        }

        $messageNode.slideDown(200,
            function(){
                window.setTimeout(
                    function(){
                        $messageNode.slideUp(200,
                            function() {
                                $messageNode.remove();
                            }
                    );
    ...
    {
        PEEP.registry['flag-panel-fb'] = PEEP.ajaxFloatBox("BASE_CMP_Flag", [entityType, entityId], {
            width: 315,
            title: PEEP.getLanguageText('base', 'flag_as')
        });
    };
    ...
    {
    	PEEP.Users.showUsers(userIds, title);
    },

    this.getActiveFloatBox = function getFloatBox()
    {
    	if ( typeof window.PEEPActiveFloatBox === 'undefined' )
    	{
    		return false;
    	}

    	return window.PEEPActiveFloatBox;
    },
     ...
     * Examples:
     * 1:
     * PEEP.loadComponent(cmpClass, '.place_to_node');
     *
     * 2:
     * PEEP.loadComponent(cmpClass, function(){
     *     //onReady event
     *     //Add the html somewhere
     * });
     *
     * 3:
     * PEEP.loadComponent(cmpClass, [ p1, p2, ... ], '.place_to_node');
     *
     * 4:
     * PEEP.loadComponent(cmpClass, [ p1, p2, ... ], function(html){
     *     //onReady event
     *     //Add the html somewhere
     * });
     *
     * 5:
     * PEEP.loadComponent(cmpClass, [ p1, p2, ... ],
     * {
     *     onLoad: function(){},
     *     onError: function(){},
     *     onComplete: function(){},
    ...
    this.loadComponent = function( cmpClass, p1, p2 )
    {
        ...
        if ( options.place )
        {
            $preloader = $('<div class="peep_ajaxloader_preloader peep_preloader_content ' + options.addClass + '"></div>');
        ...
        var ajaxOptions = {
            ...
            success: function(markup)
            {
                ...
                PEEP.bindAutoClicks($contentHtml);
                PEEP.bindTips($contentHtml);
                ...
                {
                    PEEP.addScript(markup.beforeIncludes, options.scope, options.context);
                }
                ...
                if (markup.scriptFiles)
                {
                    ...
                    PEEP.addScriptFiles(markup.scriptFiles, function()
                    {
                        ...
                        {
                            PEEP.addScript(markup.onloadScript, options.scope, options.context);
                            options.onLoad();
                        }
                    });
                }
    ...
    this.ajaxFloatBox = function(cmpClass, params, options)
    {
    	...
    	var floatBox = new PEEP_FloatBox({
            $title: options.title,
            width: options.width,
	...
	{
        var fb = PEEP.ajaxFloatBox("BASE_CMP_LanguageValueEdit", [prefix, key, true], {width: 520, title: this.getLanguageText('admin', 'edit_language')});

        PEEP.bind("admin.language_key_edit_success", function( e ) {
            fb.close();
            PEEP.unbind("admin.language_key_edit_success");
            success(e);
        });
	};
    ...
    this.bindAutoClicks = function(context){
        ...
        $.each(autoClicks, function(i,o){
            ...
            .bind('focus.auto_click', {context:context},
                function(e){
                    $('.peep_submit_auto_click', e.data.context).show();
            ...
            .bind('blur.auto_click', {context:context},
                function(e){
                    if( $(this).hasClass('invitation') ){
                        $('.peep_submit_auto_click', e.data.context).hide();
                    }
                }
            );*/
        });
    ...
    this.initWidgetMenu = function( items ){
        ...
        $.each( items, function(key, value){
                if( $toolbarCont === null ){
                    $contex = $('#'+value['contId']).closest('.peep_box, .peep_box_empty');
                    $toolbarCont = $('.peep_box_toolbar_cont', $contex);
                }
                        ...
                        if( e.data.value.toolbarId != undefined ){
                            if( e.data.value.toolbarId ){
                                if( $toolbarCont.length === 0 ){
                                    $toolbarCont = $('<div class="peep_box_toolbar_cont"></div>');
    ...
    this.showTip = function( $el, params ){
        params = params || {};
        params = $.extend({side:'bot', show:null, width:null, timeout:0, offset:5, hideEvent: null}, params);

        var showTipN = function(){

            var $rootEl = $el.data('peepTip');
            ...
            var tod = setTimeout( function(){
                $el.data('peepTip').show( 1,
                    function(){
                        if( params.hideEvent ){
                            $el.bind(params.hideEvent, function(){PEEP.hideTip($el)});
                        }
                        $el.data('peepTipStatus', true);
                        if( $el.data('peepTipHide') == true ){
                            PEEP.hideTip($el);
                        }
                    }
                );
        ...
        if( params.show != null ){
            ...
            showContent = '<span class="peep_tip_title">'+$el.attr('title')+'</span>';
        }

        var $rootEl = $('<div class="peep_tip peep_tip_'+params.side+'"></div>').css({display:'none'}).append($('<div class="peep_tip_arrow"><span></span></div><div class="peep_tip_box">'+ showContent +'</div>'));
    ...
    this.hideTip = function( $el ){
        ...
        if( $el.data('peepTip') && $el.data('peepTipStatus') == true ){
            $el.data('peepTip').hide();
            $el.data('peepTipStatus', false);
            $el.data('peepTipHide', false);
        }
    };

    this.bindTips = function( $context ){
      //$('*[title]', $context).each( function(i, o){$(o).hover(function(){PEEP.showTip($(this), {timeout:200})}, function(){PEEP.hideTip($(this))})});
      $('*[title]', $context).each( function(i, o){
          $(o).on('mouseenter', function(){ PEEP.showTip($(this), {timeout:200}); });
          $(o).on('mouseleave', function(){ PEEP.hideTip($(this)); });
      });
    };
    ...
    {
        var floatBox = new PEEP_FloatBox({layout: 'empty'});
...
jQuery.fn.extend({
	inprogress: function() {
		this.each(function()
		{
			var $this = jQuery(this).addClass('peep_inprogress');
			this.disabled = true;
	...
	activate: function() {
		this.each(function()
		{
			var $this = jQuery(this).removeClass('peep_inprogress');
			this.disabled = false;
 ...
 */
function PEEP_FloatBox(options)
{
    var fl_box = this;
    var fb_class;
    this.parentBox = PEEP.getActiveFloatBox();
    ...
    if (typeof document.body.style.maxHeight === 'undefined') { //if IE 6
            ...
            { //iframe to hide select elements in ie6
                ...
                fb_class = PEEP_FloatBox.detectMacXFF() ? 'floatbox_overlayMacFFBGHack' : 'floatbox_overlayBG';
    ...
    window.PEEPActiveFloatBox = this;
}

PEEP_FloatBox.version = 3;
PEEP_FloatBox.detectMacXFF = function()
{
    var userAgent = navigator.userAgent.toLowerCase();
    return (userAgent.indexOf('mac') != -1 && userAgent.indexOf('firefox') != -1);
}

PEEP_FloatBox.prototype = {
    ...
    /*showPreloader: function( preloaderContent )
    {
        ...
        this.$canvas.addClass('peep_floatbox_loading');
    ...
    {
        this.$canvas.removeClass('peep_floatbox_loading');
    },
    ...
    close: function()
    {
        ...
        window.PEEPActiveFloatBox = this.parentBox;
...
var PeepFormElement = function( id, name ){
    this.id = id;
    this.name = name;
...
PeepForm.prototype = {
    ...
    submitForm: function(){
        ...
        var buttons = $('input[type=button], input[type=submit], button', '#' + this.id).addClass('peep_inprogress');

        if( this.ajax ){
            PEEP.inProgressNode(buttons);
            var postString = '';
...
var PeepTextField = function( id, name, invitationString ){
...
var PeepTextArea = function( id, name, invitationString ){
    var formElement = new PeepFormElement(id, name);
    if( invitationString ){
        addInvitationBeh(formElement, invitationString);
    }
...
var PeepWysiwyg = function( id, name, invitationString ){
    var formElement = new PeepFormElement(id, name);
...
var PeepRadioField = function( id, name ){
    var formElement = new PeepFormElement(id, name);
...
var PeepCheckboxGroup = function( id, name ){
    var formElement = new PeepFormElement(id, name);
...
var PeepCheckboxField = function( id, name ){
    var formElement = new PeepFormElement(id, name);
...
var PeepRange = function( id, name ){
    var formElement = new PeepFormElement(id, name);
...
var PeepAvatarField = function( id, name, params ){
    var formElement = new PeepFormElement(id, name);
    var $preview = $(formElement.input).closest(".peep_avatar_field").find(".peep_avatar_field_preview");
...
/* Comments */
var PeepComments = function( params ){
    $.extend(this, params);
    this.$cmpContext = $('#' + this.contextId);
    var self = this;
    var sleeping = false;
    this.eventParams = {entityType:self.entityType, entityId:self.entityId, customId:self.customId};
    this.submitHandler = function(){PEEP.error('FORM IS NOT INITIALIZED!');};
    this.attachmentInProgressHandler = function(){PEEP.error(params.labels.attachmentLoading);};
    ...
    PEEP.bind('base.comments_wakeup', 
        function(data){
            if( checkAddress(data) ){
                sleeping = false;
    ...
    PEEP.bind('base.comments_destroy', 
        function(data){
            if( checkAddress(data) ){
    ...
    this.$formWrapper = $('.peep_comments_form_wrap', this.$cmpContext);
    this.$formWrapperPre = $('.peep_comments_form_wrap_pre', this.$cmpContext);
    this.$hiddenBtnCont = $('.comments_hidden_btn', this.$cmpContext);
    this.$commentsInputCont = $('.peep_comments_item_info', this.$formWrapper);
    this.initTextarea();
    this.attachmentInfo = false;
    this.oembedInfo = false;

    PEEP.bind('base.comments_list_init',
        function(data){
            if( sleeping ) return;
            if( checkAddress(data) ){
    ...
    PEEP.bind('base.add_photo_attachment_submit',
        function(data){
            ...
            if( data.uid == self.attchUid ){
                ...
                PEEP.trigger('base.comment_button_show', self.eventParams);
            }
        }
    );
    ...
    PEEP.bind('base.attachment_added',
        function(data){
            ...
            if( data.uid == self.attchUid ){                
                ...
                PEEP.trigger('base.comment_attachment_added', self.eventParams);
            }
        }
    );
    ...
    PEEP.bind('base.attachment_deleted',
        function(data){
            ...
            if( data.uid == self.attchUid ){
                ...
                PEEP.trigger('base.comment_attachment_deleted', self.eventParams);
            }
        }
    );

    PEEP.bind('base.move_comments_form',
        function(data){
            if( sleeping ) return;
...
PeepComments.prototype = {
    repaintCommentsList: function(data){
        if(data.error){
            PEEP.error(data.error);
            return;
        }
        //PEEP.trigger('base.comments_list_update', {entityType: data.entityType, entityId: data.entityId, id:this.uid});
        $('.comments_list_cont', this.$cmpContext).empty().append($(data.commentList));        
        PEEP.addScript(data.onloadScript);
    },
    ...
    sendMessage: function( message ){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                PEEP.trigger('base.photo_attachment_uid_update', {uid:self.attchUid, newUid:data.newAttachUid});
                self.eventParams.commentCount = data.commentCount;
                PEEP.trigger('base.comment_added', self.eventParams);
                self.attchUid = data.newAttachUid;
                
                self.$formWrapper.removeClass('peep_preloader');
    ...
    initTextarea: function(){
        ...
        this.realSubmitHandler = function(){
            ...
            if( this.mediaAllowed ){
                PEEPLinkObserver.getObserver(self.textAreaId).resetObserver();
            }
            self.$attchCont.empty();
            PEEP.trigger('base.photo_attachment_reset', {pluginKey:self.pluginKey, uid:self.attchUid});
            PEEP.trigger('base.comment_add', self.eventParams);
            
            self.$formWrapper.addClass('peep_preloader');
        ...
        this.$textarea
                    ...
                    if( e.which === 13 && !e.shiftKey ){
                        var textBody = $(this).val();
                         ...
                         if ( $.trim(textBody) == '' && !self.attachmentInfo && !self.oembedInfo ){
                             PEEP.error(self.labels.emptyCommentMsg);
                            return false;
                         }
            ...
            .one('focus', function(){$(this).removeClass('invitation').val('').autosize({callback:function(data){PEEP.trigger('base.comment_textarea_resize', self.eventParams);}});});

       this.$hiddenBtnCont.unbind('click').click(function(){self.submitHandler();});

       if( this.mediaAllowed ){
           PEEPLinkObserver.observeInput(this.textAreaId, function( link ){
            if( !self.attachmentInfo ){
                 self.$attchCont.html('<div class="peep_preloader" style="height: 30px;"></div>');
                 this.requestResult( function( r ){
                     self.$attchCont.html(r);
                     self.$hiddenBtnCont.show();

                     PEEP.trigger('base.comment_attach_media', {})
                 });
...
var PeepCommentsList = function( params ){
	this.$context = $('#' + params.contextId);
	$.extend(this, params, peepCommentListCmps.staticData);
        this.$loader = $('.peep_comment_list_loader', this.$context);
}

PeepCommentsList.prototype = {
	init: function(){
	var self = this;
        $('.peep_comments_item', this.$context).hover(function(){$('.cnx_action', this).show();$('.peep_comments_date_hover', this).hide();}, function(){$('.cnx_action', this).hide();$('.peep_comments_date_hover', this).show();});
        this.$loader.one('click',
            function(){
                self.$loader.addClass('peep_preloader');
                $('a', self.$loader).hide();
...
//        PEEP.bind('base.comments_list_update',
//            function(data){
...
//        );
        ...
        PEEP.trigger('base.comments_list_init', {entityType: this.entityType, entityId: this.entityId}, this);

        PEEP.bind('base.comment_add', function(data){ if( data.entityType == self.entityType && data.entityId == self.entityId ) self.initialCount++ });
            ...
            function(i,o){
                $('#'+i).click(
                    function(){
                        ...
                        {
                           $(this).closest('div.peep_comments_item').slideUp(300, function(){$(this).remove();});
                            ...
                            $.ajax({
                                ...
                                success : function(data){
                                    ...
                                    PEEP.trigger('base.comment_delete', eventParams, this);
                                }
                            });
                        }
                    }
		...
		$.ajax({
            ...
            success : function(data){
               if(data.error){
                ...
                self.$loader.removeClass('peep_preloader');
                $('a', self.$loader).hide();
                self.$context.replaceWith(data.commentList);
                PEEP.addScript(data.onloadScript);
            },
            error : function( XMLHttpRequest, textStatus, errorThrown ){
                PEEP.error('Ajax Error: '+textStatus+'!');
                throw textStatus;
            }
        });
	}
...
peepCommentCmps = {};
peepCommentListCmps = {items:{},staticData:{}};

var PeepRate = function( params ){
    ...
    updateRate: function( rate ){
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                PEEP.trigger('base.rate_update', {entityType: self.entityType, entityId: self.entityId, rate: rate, ownerId: self.ownerId}, this);
            },
            error : function( XMLHttpRequest, textStatus, errorThrown ){
...
PEEPLinkObserver =
{
    ...
    {
        this.observers[inputId] = new PEEPLinkObserver.handler(inputId, callBack);
    },
...
PEEPLinkObserver.handler = function( inputId, callBack )
{
...
PEEPLinkObserver.handler.prototype =
{
    ...
    requestResult: function( callback, link )
    {
        ...
        $.ajax({
            type: 'POST',
            url: PEEP.ajaxAttachmentLinkRsp,
            data: {"url": link},
            dataType: 'json',
            success: function( r )
            {
                if ( r.content )
                {
                    ...
                    if ( r.content.js )
                    {
                        PEEP.addScript(r.content.js);
                    }
                }

                if ( r.attachment && PEEP_AttachmentItemColletction[r.attachment] )
                {
                    PEEP_AttachmentItemColletction[r.attachment].onChange = function(oembed){
...
PEEP_AttachmentItemColletction = {};

PEEP_Attachment = function(uniqId, data)
{
    var self = this;
    ...
    //PEEP.resizeImg(this.$('.EQ_AttachmentImageC'),{width:'150'});

    this.$('.PEEP_AttachmentSelectPicture').click(function()
    {
        self.showImageSelector();
    });

    this.$('.PEEP_AttachmentDelete').click(function()
    {
        $(self.node).remove();
...
PEEP_AttachmentProto = function()
{
    ...
    this.showImageSelector = function()
    {
        ...
        $contents = this.$('.PEEP_AttachmentPicturesFbContent')

        fb = new PEEP_FloatBox({
            $title: this.$('.PEEP_AttachmentPicturesFbTitle'),
            $contents: $contents,
            width: 520
        });

        $contents.find('.PEEP_AttachmentPictureItem').unbind().click(function()
        {
            var img = $('img', this);
    ...
    this.changeImage = function( url )
    {
        ...
        original = this.$('.PEEP_AttachmentImage');
...
PEEP_Attachment.prototype = new PEEP_AttachmentProto();
...
PEEP_PingCommand = function( commandName, commandObject, stack )
{
...
PEEP_PingCommand.PROTO = function()
{
...
PEEP_PingCommand.prototype = new PEEP_PingCommand.PROTO();
...
PEEP_Ping = function()
{
    ...
    this.addCommand = function( commandName, commandObject )
    {
        ...
        _commands[commandName] = new PEEP_PingCommand(commandName, commandObject, _stack);
...
PEEP_Ping.getInstance = function()
{
    if ( !PEEP_Ping.pingInstance )
    {
        PEEP_Ping.pingInstance = new PEEP_Ping();
    }

    return PEEP_Ping.pingInstance;
}



PEEP.getPing = function()
{
    return PEEP_Ping.getInstance();
};

/* Add command example
PEEP.getPing().addCommand('ajaxim', {
    params: {
        p1: 1,
        p2: 3
    },
...
PEEP.addScroll = function( node, settings )
{
    settings = settings || {};
    settings = $.extend({}, PEEP.Scroll.settings, settings);
    ...
    if ( !$node.hasClass('peep_scrollable') )
    {
        $node.addClass('peep_scrollable');
    }
...
PEEP.removeScroll = function( node )
{
    var $node = $(node);
    ...
    $node.removeClass('peep_scrollable');
};

PEEP.updateScroll = function( node, settings )
{
    var jsp = $(node).data('jsp');
...
PEEP.addScrolls = function( context )
{
    context = context || null;

    $('.peep_scrollable', context).each(function()
    {
        PEEP.addScroll(this);
    });
};

PEEP.removeScrolls = function( context )
{
    context = context || null;

    $('.peep_scrollable', context).each(function()
    {
        PEEP.removeScroll(this);
    });
};

PEEP.updateScrolls = function( context )
{
    context = context || null;

    $('.peep_scrollable', context).each(function()
    {
        PEEP.updateScroll(this);
    });
};
...
PEEP.Users = null;
PEEP_UsersApi = function( _settings )
{
    var _usersApi = this;
    ...
    var _query = function(command, params, callback) {
        callback = callback || function( r ) {
            if ( !r ) return;
            if (r.info) PEEP.info(r.info);
            if (r.error) PEEP.error(r.error);
        };
    ...
    this.deleteUser = function(userId, callBack, showMessage)
    {
        ...
        floatBox = PEEP.ajaxFloatBox('BASE_CMP_DeleteUser', [{userId: userId, showMessage : showMessage && redirectUrl}],
        {
            title: PEEP.getLanguageText('base', 'delete_user_confirmation_label'),
            iconClass: 'peep_ic_add',
            scope: {
    ...
    this.showUsers = function(userIds, title)
    {
    	title = title || PEEP.getLanguageText('base', 'ajax_floatbox_users_title');

    	PEEP.ajaxFloatBox('BASE_CMP_FloatboxUserList', [userIds], {iconClass: "peep_ic_user", title: title, width: 470});
    },
    ...
    this.blockUserWithConfirmation = function( userId, confirmationCallback, callback ) {
        var floatBox = PEEP.ajaxFloatBox("BASE_CMP_BlockUser", [userId], {
            scope: {
