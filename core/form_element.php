...
abstract class FormElement
{
    ...
    {
        if ( !$validator instanceof PEEP_Validator )
        {
    ...
    {
        $jsString = "var formElement = new PeepFormElement(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
abstract class InvitationFormElement extends FormElement
{
    ...
    public function __construct( $name )
    {
        ...
        $this->setInvitation(PEEP::getLanguage()->text('base', 'form_element_common_invitation_text'));
    }
...
class TextField extends InvitationFormElement
{
    ...
    public function getElementJs()
    {
        $jsString = "var formElement = new PeepTextField(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ", " . json_encode(( $this->getHasInvitation() ? $this->getInvitation() : false)) . ");";
...
class DateField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'date_field.js');

        PEEP::getDocument()->addOnloadScript(" if( window.date_field == undefined ) { window.date_field = {}; } window.date_field['" . $this->getId() . "'] = new DateField( '" . ( $this->getName() ) . "' ); ");
        ...
        $config = PEEP::getConfig()->getValue('base', 'date_field_format');
        ...
        if ( $config === 'dmy' )
        {
            $result = '<div class="' . $this->getAttribute('name') . '">
                            ...
                            <div class="peep_inline peepm_inline">' . UTIL_HtmlTag::generateTag('select', $dayAttributes, true, $dayOptionsString) . '</div>
                            <div class="peep_inline peepm_inline">' . UTIL_HtmlTag::generateTag('select', $monthAttributes, true, $mounthOptionsString) . '</div>
                            <div class="peep_inline peepm_inline">' . UTIL_HtmlTag::generateTag('select', $yearAttributes, true, $yearOptionsString) . '</div>
                        </div>';
        }
        else
        {
...
class Submit extends FormElement
{
    ...
    public function __construct( $name, $decorator = 'button' )
    {
        ...
        $this->setValue(PEEP::getLanguage()->text('base', 'form_element_submit_default_value'));
    ...
    public function renderInput( $params = null )
    {
        ...
        {
            $finalMarkup = PEEP::getThemeManager()->processDecorator('button', $params);
        }
        else
        {
...
class RadioField extends FormElement
{
    ...
    public function getElementJs()
    {
        $js = "var formElement = new PeepRadioField(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
    ...
    public function renderInput( $params = null )
    {
        ...
        $renderedString = '<ul class="peep_radio_group clearfix">';
...
//TODO rename getter or setter
    ...
    public function getElementJs()
    {
        $js = "var formElement = new PeepCheckboxGroup(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
    ...
    public function renderInput( $params = null )
    {
        ...
        $renderedString = '<ul class="peep_checkbox_group clearfix">';
...
class Selectbox extends InvitationFormElement
{
    ...
    public function __construct( $name )
    {
        ...
        $this->setInvitation(PEEP::getLanguage()->text('base', 'form_element_select_field_invitation_label'));
...
class CheckboxField extends FormElement
{
    ...
    public function getElementJs()
    {
        $jsString = "var formElement = new PeepCheckboxField(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
class Multiselect extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        return
            '<table class="peep_multiselect" id="' . $this->getId() . '">
                <tr>
                    <td>' . $chicesSelectMarkup . '</td>
                    <td><input type="button" class="select" value="" /><br /><br /><br /><input type="button" class="deselect peep_ic_left_arrow" value="" /></td>
    ...
    public function getElementJs()
    {
        ...
        PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'multiselect_field.js');

        $jsString = "var formElement1 = new PeepFormElement('" . $this->getId() . "', '" . $this->getName() . "');
...
class SuggestField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        {
            $this->addAttribute('value', $this->value);
            $this->addAttribute('class', 'peep_inputready');
        }

        $this->addAttribute('type', 'text');

        $scriptUrl = PEEP::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery-suggest.js';
        PEEP::getDocument()->addScript($scriptUrl);

        $js = '
            var $input = $("#' . $this->getId() . '");
            var initialLabel = $input.val();
            $input.parent().find(".peep_suggest_invitation").click(function(){ $input.focus(); });
            $input.suggest(' . $this->responderUrl . ', {autoSuggest: true, minchars: ' . $this->minChars . ', onFocus: function(first){
                    $(this).removeClass("peep_inputready");
        ...
        return '<div class="peep_suggest_field">' . UTIL_HtmlTag::generateTag('input', $this->attributes) . '<div class="peep_suggest_invitation"></div></div>';
    }
}
...
class TagsField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $tagTpl = '<div class="tags-field-tag-cont" style="margin: 2px 2px 0pt 0pt; padding: 2px 5px; background: rgb(204, 204, 204) none repeat scroll 0% 0%; float: left;"><span class="tags-field-tag">${tag}</span> <a class="peep_lbutton peep_red tags-field-del-tag">x</a></div>';
        ...
        $tpl = '<span id=${id}>' . '${hidden}' . '<input class="peep_text" type="text">' . '${tags}' . '</span>';
        ...
        PEEP::getDocument()->addScript($plugin->getStaticJsUrl() . 'tags_field.js');
...
{
    const CAPTCHA_PREFIX = 'peep_captcha_';
    ...
    public function renderInput( $params = null )
    {
        ...
        $captchaUrl = PEEP::getRouter()->urlFor('BASE_CTRL_Captcha', 'index');
        $captchaResponderUrl = PEEP::getRouter()->urlFor('BASE_CTRL_Captcha', 'ajaxResponder');
        $captchaClass = $this->getName() . '_' . $this->getId();
        $uniqueId = md5(time());

        $jsDir = PEEP::getPluginManager()->getPlugin("base")->getStaticJsUrl();
        PEEP::getDocument()->addScript($jsDir . "captcha.js");

        $string = ' window.' . $this->jsObjectName . ' = new PEEP_Captcha( ' . json_encode(array('captchaUrl' => $captchaUrl,
                'captchaClass' => $captchaClass,
                'captchaId' => $this->getId(),
                'responderUrl' => $captchaResponderUrl
            )) . ');';

        PEEP::getDocument()->addOnloadScript($string);

        return '<div class="' . $captchaClass . '">
                    <div class="peep_automargin clearfix" style="width: 230px;">
                            <div class="peep_left"><img src="' . $captchaUrl . '" id="siimage"></div>
                            <div class="peep_right" style="padding-top: 21px;"><span class="ic_refresh peep_automargin" id="siimage_refresh" style="cursor:pointer;"></span></div>
                    </div>
...
class AgeRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'age_range_field.js');
        PEEP::getDocument()->addOnloadScript(" window." . $this->getName() . " = new AgeRangeField( " . json_encode($this->getName()) . ", " . json_encode($this->minAge) . ", " . ( $this->maxAge ) . " ); ");
        ...
        $result = '<div id="' . $this->getAttribute('id') . '"class="' . $this->getAttribute('name') . '">
                        <div class="peep_range_from peep_inline ">' . $language->text('base', 'form_element_from') . '</div>
                        <div class="peep_inline">' . UTIL_HtmlTag::generateTag('select', $fromAgeAttrs, true, $fromAgeOptionsString) . '</div>
                        <div class="peep_range_to peep_inline">' . $language->text('base', 'form_element_to') . '</div>
                        <div class="peep_inline">' . UTIL_HtmlTag::generateTag('select', $toAgeAttrs, true, $toAgeOptionsString) . '</div>
                        <div class="peep_range_label peep_inline">' . $language->text('base', 'form_element_age_range') . '</div>
                    </div>';
...
class MatchAgeRange extends AgeRange
{
    ...
    public function getElementJs()
    {
        $js = "var formElement = new PeepRange(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
//    }
    ...
    public function renderInput( $params = null )
    {
        ...
        PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'range_field.js');
        PEEP::getDocument()->addOnloadScript(" window." . $this->getName() . " = new RangeField( '" . ( $this->getName() ) . "', " . ( $this->minValue ) . ", " . ( $this->maxValue ) . " ); ");
...
class DateRange extends FormElement  implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $result = '<div id="' . $this->getAttribute('id') . '" class="' . $this->getAttribute('name') . '">
                       ' . $language->text('base', 'form_element_from') . '  <div class="peep_inline">' . ( $this->minDate->renderInput() ) . '</div>
                       ' . $language->text('base', 'form_element_to') . '
                       <div class="peep_inline">' . ( $this->maxDate->renderInput() ) . '</div>
                    </div>';
...
class BillingGatewaySelectionField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( $gateways )
        {
            ...
            {
                /* @var $adapter PEEP_BillingAdapter */
            ...
            $cont_id = $id . '-cont';
            $renderedString .= '<ul class="peep_billing_gateways clearfix" id="' . $cont_id . '">';
            ...
            foreach ( $paymentOptions as $option )
            {
                ...
                if ( $gatewaysNumber == 1 )
                {
                    $renderedString .= '<li style="display: inline-block; padding-right: 20px;">' . PEEP::getLanguage()->text('base', 'billing_pay_with') . '</li>';
        ...
        {
            $renderedString = PEEP::getLanguage()->text('base', 'billing_no_gateways');
        }
...
class YearRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'age_range_field.js');
        PEEP::getDocument()->addOnloadScript(" window." . $this->getName() . " = new AgeRangeField( " . json_encode($this->getName()) . ", " . json_encode($this->minYear) . ", " . json_encode($this->maxYear) . " ); ");
        ...
        $result = '<div id="' . $this->getAttribute('id') . '"class="' . $this->getAttribute('name') . '">
                        <div class="peep_range_from peep_inline">' . $language->text('base', 'form_element_from') . '</div>
                        <div class="peep_inline">' . UTIL_HtmlTag::generateTag('select', $fromYearAttrs, true, $fromAgeOptionsString) . '</div>
                        <div class="peep_range_to peep_inline">' . $language->text('base', 'form_element_to') . '</div>
                        <div class="peep_inline">' . UTIL_HtmlTag::generateTag('select', $toYearAttrs, true, $toAgeOptionsString) . '</div>
                        <div class="peep_range_label peep_inline">' . $language->text('base', 'form_element_year_range') . '</div>
                    </div>';
...
class MobileWysiwygTextarea extends Textarea
{
    ...
    public function __construct( $name, $pluginKey = 'blog', array $buttons = array() )
    {
        ...
        $stringValidator->setErrorMessage(PEEP::getLanguage()->text('base', 'text_is_too_long', array('max_symbols_count' => 50000)));
	...
	if ( PEEP::getRegistry()->get('baseWsInit') === null )
        {
            if ( in_array(BOL_TextFormatService::WS_BTN_IMAGE, $this->buttons) )
            {
                PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.html5_upload.js');
            }

            PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'suitup.jquery.js?a='.time());
            PEEP::getDocument()->addStyleSheet(PEEP::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'suitup.css?a='.time());

            // register js langs
            PEEP::getLanguage()->addKeyForJs('base', 'ws_button_label_link');
            PEEP::getLanguage()->addKeyForJs('base', 'ws_button_label_video');
            PEEP::getLanguage()->addKeyForJs('base', 'ws_error_video');

            PEEP::getRegistry()->set('baseWsInit', true);
        }

        $this->addAttribute('class', 'peepm_suitup_wyswyg');
        $js = UTIL_JsGenerator::newInstance();

        $js->addScript('$("#" + {$uniqId}).suitUp({$buttons}, {$imageUploadUrl}, {$embedUrl}).show();', array(
            'buttons' => $this->buttons,
            'uniqId' => $this->getId(),
            'imageUploadUrl' => PEEP::getRouter()->urlFor('BASE_CTRL_MediaPanel', 'ajaxUpload', array(
                'pluginKey' => $this->pluginKey
            )),
            'embedUrl' => PEEP::getRouter()->urlFor('BASE_MCTRL_Oembed', 'getAjaxEmbedCode')
        ));
...
class WysiwygTextarea extends InvitationFormElement
{
    ...
    public function __construct( $name, array $buttons = null, $init = true )
    {
        ...
        if ( PEEP::getRequest()->isMobileUserAgent() )
        {
    ...
    public function getElementJs()
    {
        ...
        $jsString = "var formElement = new PeepWysiwyg(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ", " . json_encode($invitation) . ");            
        ";
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( PEEP::getRegistry()->get('baseWsInit') === null )
        {
            ...
            $array = array(
                'editorCss' => PEEP::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'htmlarea_editor.css',
                'themeImagesUrl' => PEEP::getThemeManager()->getCurrentTheme()->getStaticImagesUrl(),
                'imagesUrl' => PEEP::getRouter()->urlFor('BASE_CTRL_MediaPanel', 'index', array('pluginKey' => 'blog', 'id' => '__id__')),
                'labels' => array(
                    'buttons' => array(
                ...
                'buttonCode' => PEEP::getThemeManager()->processDecorator('button', array('label' => '#label#', 'class' => 'peep_ic_add mn_submit')),
            ...
            PEEP::getDocument()->addScriptDeclarationBeforeIncludes($script);
            PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'htmlarea.js');
            PEEP::getDocument()->addStyleSheet(PEEP::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'htmlarea.css');
            PEEP::getRegistry()->set('baseWsInit', true);
        }
        ...
        if ( $this->init )
        {
            PEEP::getDocument()->addOnloadScript("$('#{$this->getId()}').htmlarea( " . json_encode($params) . " );");
        }
...
class TagsInputField extends FormElement 
{
    ...
    public function __construct( $name )
    {
        parent::__construct($name);
        $this->value = array();
        $this->invLabel = PEEP::getLanguage()->text('base', 'tags_input_field_invitation');
    ...
    public function renderInput( $params = null )
    {
        ...
        PEEP::getDocument()->addScript(PEEP::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.tagsinput.js');
...
var formElement = new PeepFormElement('" . $this->getId() . "', '" . $this->getName() . "');";
